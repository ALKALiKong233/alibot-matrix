...
async def repo(room, message):
    ...
    if match.is_not_from_this_bot() and match.prefix() and match.command("sync"):
        ...
        if not match.args():
            sync = os.popen(str(buildinfo['proxy']) + ' repo sync --force-sync -j$(nproc --all) --no-tags --no-clone-bundle --current-branch | tee sync.log ').read()
        else:
            sync = os.popen(str(buildinfo['proxy']) + ' repo sync --force-sync -j$(nproc --all) --no-tags --no-clone-bundle --current-branch ' + " ".join(match.args()) + ' | tee sync.log ').read()
        await alibuildbot.api.send_text_message(room.room_id, sync)
